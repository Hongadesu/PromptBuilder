...
import { useGlobalStore, useTemplateDetailStore as useStore } from '@/stores';
import { PywebviewApi } from '@/apis';
import { cn } from '@/modules/utils';
import scrollbarStyles from '@/styles/Scrollbar.module.css';

const KeyInput = React.memo(
  ({ promptKey, promptValue }: { promptKey: string; promptValue: string }) => {
      ...
      <div className='flex flex-col gap-2'>
        <div className='flex gap-2'>
          ...
          <button
            type='button'
            tabIndex={0}
            className='ml-auto flex cursor-pointer items-center justify-center rounded-md border border-[var(--border)] bg-transparent p-1.5 text-[var(--border)] hover:bg-[var(--border)] hover:text-[var(--on-border)]'
            onClick={() => {
        ...
        <textarea
          id={`input-${promptKey}`}
          tabIndex={0}
          className={`${scrollbarStyles.surfaceScrollable} h-20 min-w-24 resize-none rounded-md border border-[var(--border)] bg-[var(--bg)] px-2 py-1 text-sm text-[var(--on-bg)] focus-visible:border-[var(--primary)] focus-visible:outline-1 focus-visible:outline-[var(--primary)]`}
...
function Detail({ detailClassName }: { detailClassName?: string }) {
      ...
      <div className='flex flex-1/2 shrink-0 grow-0 flex-col gap-2 overflow-hidden'>
        ...
        >
          {Object.entries(keyValueMap).map(([key, value]) => (
            <KeyInput key={key} promptKey={key} promptValue={value} />
          ))}
        </div>
      </div>
    </div>
...
TemplateDetail.Title = Title;
TemplateDetail.Description = Description;
TemplateDetail.Detail = Detail;
TemplateDetail.DetailTabStyle = DetailTabStyle;
TemplateDetail.DetailTabButtons = DetailTabButtons;
TemplateDetail.ButtonSetA = ButtonSetA;
TemplateDetail.ButtonSetB = ButtonSetB;
TemplateDetail.DeleteButton = DeleteButton;

export { TemplateDetail, useDetailTabStore };
