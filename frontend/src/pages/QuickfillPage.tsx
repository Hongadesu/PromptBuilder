      ...
      {isTall && (
        <div className='mx-4 mt-4 flex gap-2 max-lg:mx-auto max-lg:w-11/12'>
          ...
          <Button
            variant='secondary'
            className='h-8 px-2'
            onClick={() => navigate('/test/quickfill')}
          >
            新增模板
          </Button>
        </div>
      )}
      <QuickfillListPanel className='flex-1 overflow-hidden' />
      {isTall ? (
...
function QuickfillListPanel({ className }: { className?: string }) {
  const navigate = useNavigate();
  const { search } = useLocation();
  const query = new URLSearchParams(search);
  const currPage = Number(query.get('page')) || 1;
  const templateList = useQuickfillListStore((state) => state.templateList);
  const maxPage = useQuickfillListStore((state) => state.maxPage);
  const setDetail = useQuickfillDetailStore.getState().setDetail;

  const selectedId = usePage((state) => state.selectedId);
  const setSelectedId = usePage.getState().setSelectedId;

  useEffect(() => {
    useGlobalStore.getState().getQuickfillItems(currPage, NUM_PER_PAGE);
  }, [currPage]);
...
function InfoPanel({ className }: { className?: string }) {
  const id = useQuickfillDetailStore((state) => state.id);
  const setSelectedId = usePage.getState().setSelectedId;
  ...
  return (
    ...
    >
      {id ? (
        <QuickfillDetailInfo onClose={() => setSelectedId(undefined)} />
      ) : (
