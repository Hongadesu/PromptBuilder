...
import { useGlobalStore, useQuickfillEditorStore as useStore } from '@/stores';
...
import {
  ...
  LockableKeyInput,
  SaveBtn,
} from '@/components/prompt-test';
import { useIsTall } from '@/hooks';
import { cn } from '@/modules/utils';
import styles from '@/styles/Scrollbar.module.css';

/* Quickfill 提示詞模板新增/測試頁面 */
export function QuickfillTestPage({ className }: { className?: string }) {
      ...
      <div className='relative mx-auto flex h-full min-h-96 w-5/6 flex-col py-6'>
        ...
        >
          ...
          <Button
            variant='secondary'
            className='h-8 px-2'
            onClick={() => navigate('/test/default', { replace: true })}
          >
            Basic
          </Button>
        </div>
        ...
        <div className='flex items-center justify-center gap-2 py-2'>
          ...
          <SaveBtn
            onClick={() => {
              ...
              let unfrozenCount = 0;
              Array.from(keyValueMap.values()).map(
                ({ isFrozen }) => !isFrozen && unfrozenCount++,
              );
              if (unfrozenCount !== 1) {
                showToast({
                  type: 'error',
                  message: '請確保非凍結的選項僅為一項',
                });
                return;
              }
              setDialogOpen(true);
        ...
        <Dialog
          ...
          title='新增 Quickfill 提示詞模板對話'
          description='新增 Quickfill 提示詞模板對話'
          portal={
            <QuickfillAppendPortal onClose={() => setDialogOpen(false)} />
          }
        />
      </div>
    </div>
...
function QuickfillAppendPortal({ onClose }: { onClose: () => void }) {
  const titleRef = useRef<HTMLInputElement | null>(null);
  const descriptionRef = useRef<HTMLTextAreaElement | null>(null);
  const appendQuickfillItem = useGlobalStore.getState().appendQuickfillItem;
      ...
      <div className='flex gap-2'>
        ...
        <FeedbackButton
          ...
          onClick={async () => {
            ...
            for (const [key, { value, isFrozen }] of keyValueMap) {
              if (!validKeys.has(key)) {
                const message = `key: ${key} 非法`;
                showToast({ type: 'error', message });
                throw new Error(message);
              }
              param[key] = isFrozen ? value : '';
            }
            ...
            const result = await appendQuickfillItem(currentTemplate);
            if (!result) {
...
function PromptKeyInputs({ className }: { className?: string }) {
  ...
  const onSetFrozen = useStore.getState().switchFrozen;
  ...
  return (
    ...
    >
      ...
      {Array.from(keyValueMap.entries()).map(([key, { value, isFrozen }]) => (
        <LockableKeyInput
          key={key}
          promptKey={key}
          promptValue={value}
          ...
          onSetFrozen={(key: string) => {
            if (value.trim() === '' && !isFrozen) {
              showToast({ type: 'info', message: '設為凍結的選項不可為空' });
              return;
            }
            onSetFrozen(key);
          }}
