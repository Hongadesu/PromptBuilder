...
import {
  ...
  useQuickfillDetailStore as useStore,
  useTemplatePinStore,
} from '@/stores';
import { QuickDetail } from '@/features/quickfill/QuickfillDetail';

export function QuickfillDetailPage() {
  const { promptId } = useParams();
  const navigate = useNavigate();
  const [deleteOpen, setDeleteOpen] = useState(false);
  const getDetail = useGlobalStore.getState().getQuickfillDetail;
  const setEditor = useQuickfillEditorStore.getState().setEditor;
      ...
      <div className='mt-4 mx-4 max-lg:w-11/12 max-lg:mx-auto flex gap-2'>
        ...
        <Button
          ...
          onClick={() => {
            ...
            navigate('/test/quickfill');
          }}
        >
          創建相似模板
        </Button>
      </div>
      <QuickDetail className='flex flex-col gap-4 h-full max-lg:w-11/12 max-lg:mx-auto'>
        <header className='flex items-center gap-2'>
          <QuickDetail.Title />
          <QuickDetail.ButtonSetA
            className='ml-auto'
